...
const normalizeTask = function normalizeTask(tasks, analysis_type, mergedName, globalFilter) {
    ...
    tasks.map(function(task){
        ...
        if(tasks.length > 1){
            ...
            console.log(JSON.stringify(tasks, undefined, 4));
    ...
    log.trace("All normalized objects:", JSON.stringify(allRequests, undefined, 4));
...
const buildNextTasks = function(normalizedTask, response){
    ...
    log.trace("Building next task from RESPONSE object:", JSON.stringify(response.then, undefined, 4));

    response.analysis.results.map(result => {

        let nextTask = _.clone(normalizedTask.then);
        let analysisType = normalizedTask.analysis.analysis_type;
        ...
        if(response.analysis.results.length > 1) {

            //console.log(JSON.stringify(response.analysis.results, undefined, 4));
