...
const createRequestObject = function(requestParameters) {
  this.localObject = {};
  this.index = requestParameters.index
    ? requestParameters.index.toString()
    : "default";
  this.api_resource = requestParameters.api_resource
    ? requestParameters.api_resource
    : config.has("index." + this.index + ".api_resource")
        ? config.get("index." + this.index + ".api_resource")
        : config.get("app.api_resource");

  // api endpoints
  this.task_uri = config.has("index." + this.index + ".task_uri")
    ? config.get("index." + this.index + ".task_uri")
    : config.has("app.task_uri") ? config.get("app.task_uri") : "";
  this.analyze_uri = config.has("index." + this.index + ".analyze_uri")
    ? config.get("index." + this.index + ".analyze_uri")
    : config.has("app.analyze_uri") ? config.get("app.analyze_uri") : "";
  ...
  this.start = requestParameters.start
    ...
    : config.has("start")
        ? config.get("start")
        : moment.utc().subtract(30, "days").unix();
  this.end = requestParameters.end
    ? requestParameters.end
    : config.has("end") ? config.get("end") : moment.utc().unix();
  ...
  this.user = process.env.AUTH_USER ||
    (config.has("index." + this.index + ".auth.username")
      ? config.get("index." + this.index + ".auth.username")
      : config.has("index.default.auth.username")
          ? config.get("index.default.auth.username")
          : "");
  this.pass = process.env.AUTH_KEY ||
    (config.has("index." + this.index + ".auth.api_key")
      ? config.get("index." + this.index + ".auth.api_key")
      : config.has("index.default.auth.api_key")
          ? config.get("index.default.auth.api_key")
          : "");
  this.subscription_id = process.env.SUBSCRIPTION_ID ||
    (config.has("index." + this.index + ".subscription_id")
      ? config.get("index." + this.index + ".subscription_id")
      : config.has("index.default.subscription_id")
          ? config.get("index.default.subscription_id")
          : "");
  ...
  this.l1Target = requestParameters.l1
    ? requestParameters.l1.target
    : undefined;
  this.l1Threshold = requestParameters.l1 && requestParameters.l1.threshold
    ? requestParameters.l1.threshold
    : 200;
  this.l2Child = requestParameters.l2 || undefined;
  this.l2Target = requestParameters.l2
    ? requestParameters.l2.target
    : undefined;
  this.l2Threshold = requestParameters.l2 && requestParameters.l2.threshold
    ? requestParameters.l2.threshold
    : 200;

  log.trace(
    `Creating request. INDEX: ${this.index}, API_RESOURCE: ${this.api_resource}, ANALYSIS_TYPE: ${this.analysis_type}`
  );
