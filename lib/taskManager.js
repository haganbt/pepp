...
const loadConfigTasks = function loadConfigTasks(configTest) {
    ...
    for(let analysis_type in configObj){
        ...
        _.each(configObj[analysis_type], function (item) {
            ...
            /*
             console.log("------------------------------------------");
             console.log(JSON.stringify(normalizedTask, undefined, 4))
             console.log("------------------------------------------");
    ...
    //const requestObject = requestFactory(normalizedTask);


    log.trace("allNormalizedTasks", allNormalizedTasks);

    const allRequestObjects = [];

    allNormalizedTasks.map(function(p, q) {
        //console.log("====",p[q]);

        const requestObject = requestFactory(p[q]);

        log.trace("Built request object:", requestObject);
    ...
    return _.flatten(allRequestObjects);
};
