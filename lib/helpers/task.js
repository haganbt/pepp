...
module.exports = {
     ...
     */
    unixToHuman(unixTs) {
        const configFormat = process.env.DATE_FORMAT || (config.has("app.date_format") ? config.get("app.date_format") : "YYYY-MM-DD HH:mm:ss");
    ...
    resUnixTsToHuman(analysis_type, responsePayload) {
        try {
            if (analysis_type === "timeSeries") {
                for (const result in responsePayload) {
                    const ts = responsePayload[result].key;

                    responsePayload[result].key = this.unixToHuman(ts);
