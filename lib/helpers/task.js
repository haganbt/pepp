...
module.exports = {
    ...
    getDefaultTaskRequest: function () {
        ...
        // Build URL
        let url = config.get("app.api_base_url")
            + "v"
            + config.get("app.api_version")
            + "/pylon/"
            + config.get("app.service")
            + "/task";

        let user = process.env.AUTH_USER || (config.has('index.default.auth.username') ? config.get('index.default.auth.username') : "");
        let pass = process.env.AUTH_KEY || (config.has('index.default.auth.api_key') ? config.get('index.default.auth.api_key') : "");
        ...
        return  _.clone({
            ...
            'json': {
                'name': 'PEPP_' + uuid.v4(),
                'type': 'analysis',
                'subscription_id': process.env.ID || config.get('index.default.id'),
                'parameters': {
                    'start': start,
                    'end': end,
                    'parameters': {
    ...
    getDefaultAnalyzeRequest: function () {
        ...
        let newObj =  _.clone({
            ...
            'uri': url,
            'json': {
                'subscription_id': process.env.ID || config.has('index.default.subscription_id') ? config.get('index.default.subscription_id') : config.get('index.default.id'),
                'start': start,
                'end': end,
                'parameters': {
                    'analysis_type': 'freqDist',
        ...
        if(config.get('app.api_version') === "1.3"){
            ...
            newObj.uri = config.get("app.api_base_url") + "v1.3/pylon/analyze"

            newObj.json.id = newObj.json.subscription_id;

            delete newObj.json.subscription_id;
