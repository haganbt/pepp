...
A ```filter``` property can be set as part of a task as expected:
...
In cases where a ```filter``` is used within a custom nested task, all child tasks automatically inherit the parent ```filter``` property from the parent task. Consider the following config:
...
Both the ```gender``` and the subsequent ```topics``` tasks will all have the ```filter``` of ```fb.all.content any "Yogi"``` appended. It is also possible to specify a filter as part of a child task as follows:
...
Once again, the filter wil be appended to the task, so in this case the child task (```fb.topics.name```) will have a filter for "Yogi" AND "Booboo" applied (as well as the inherited gender filter generated by the ```then``` paramter):

```json
"filter":"(fb.author.gender == \"female\") AND (fb.all.content any \"Yogi\") AND (fb.all.content any \"Booboo\")""
```

NOTE: Setting the ```log_level``` to ```debug`` will show the reuests being generated if needed.
