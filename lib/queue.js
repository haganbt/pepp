 ...
 * @returns {*}
 */
const responseHandler = function(normalizedTask, response, configIndex, configStart, configEnd){
    ...
    if (_.has(normalizedTask, 'then')) {
        ...
        return Promise.all(nextTasks.map(function(each) {
            ...
            const requestObject = requestFactory(each, configIndex, configStart, configEnd);
 ...
 */
const queueRequest = function queueRequest(requestObj, normalizedTask, configIndex, configStart, configEnd) {
    return queue.add(function () {
        ...
        if(normalizedTask.api_resource === "analyze"){
            ...
            return responseHandler(normalizedTask, response, configIndex, configStart, configEnd);
        ...
        if(err.error && err.error.error){
            ...
            log.error("REQUEST:",JSON.stringify(requestObj, undefined, 4));
            log.error("TASK:",JSON.stringify(normalizedTask, undefined, 4));
        } else {
            log.error(err);
        }
