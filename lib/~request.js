...
const logLevel = config.get("app.log_level");const requestFactory = require("./requestFactory").requestFactory;
...
const make = function make(requestObj) {
  ...
  if (spinner.isSpinning() === false && process.env.NODE_ENV !== "test") {
    spinner.start();
  }
 ...
 * @returns {Promise}
 */
const recursivePoll = function recursivePoll(
  normalizedTask,
  requestObj,
  taskResponse
) {
      ...
      request
        ...
        .catch(err => {
          ...
          if (retryHelper.shouldRetry(getRequest) === true) {
            ...
            log.warn("GET task " + taskResponse.id + " failed (error " + err.statusCode + "). Retry " + attempts + "...");
