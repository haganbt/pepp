...
const responseHandler = function(requestObj, response){
    ...
    if (_.has(requestObj, 'then') === false && _.has(requestObj, 'cache') === false) {
        ...
        //TODO - refactor once /task API response stabalizes
...
const queueRequest = function queueRequest(requestObj) {
    return queue.add(function () {
        ...
        console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
        console.log(JSON.stringify(requestObj, undefined, 4));
        console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
        ...
        //if this is a sync task, handle the response, otherwise poll
        if(config.get('app.api_resource') === 'analyze'){
