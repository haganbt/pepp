...
const configAnalysisTags = config.has('analysisTags') ? config.get('analysisTags') : undefined;
        ...
        if(analysisType === 'freqDist') {
            ...
            if(_.has(normalizedTask,'analysis_tag')){
                ...
                nextTask.filter = "(" + normalizedTask.target + " "
                    + operator.get(normalizedTask.target) + " \"" + taskHelper.encodeJsonString(result.key) + "\")";
            }
