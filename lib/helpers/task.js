...
module.exports = {
   ...
   * @returns {*}
   */
  normalizeResponse(api_resource, response) {
    switch (api_resource) {
  ...
  /**
   * encodeJsonString - preform and encoding
   * required for submitting CSDL.
   *
   * http://dev.datasift.com/docs/platform/csdl/query-filters
   *
   * @param str
   */
   ...
   */
  compact(obj) {
    const flat = _.flatten(obj, false);
   ...
   */
  unixToHuman(unixTs) {
    const configFormat = process.env.DATE_FORMAT ||
      (config.has("app.date_format")
        ? config.get("app.date_format")
        : "YYYY-MM-DD HH:mm:ss");
   ...
   * @returns {*}
   */
  resUnixTsToHuman(analysis_type, responsePayload) {
    const results = responsePayload.analysis.results;

    try {
      if (analysis_type === "timeSeries") {
        for (let result in results) {
          const ts = results[result].key;

          results[result].key = this.unixToHuman(ts);

          log.trace("Converting unix timestamp to human:", ts);
        }
      }
    } catch (e) {
      log.error(e);
