...
function jsonToCsv(obj) {
    return new Promise(resolve => {
        ...
        // Calculate the comparator / baseline index
        ...
        if(names[0].includes('baseline')){
            baseline = stats[names[1]];
            comparator = stats[names[0]];
        }
        
        csv = "category,key,baseline_total_authors,baseline_unique_authors,baseline_index,comparator_total_authors,comparator_unique_authors,comparator_index,exp_baseline\n";

        for (let age in comparator) {
            for (var gender in comparator[age]) {
                baseline[age][gender].exp_baseline = (comparator[age][gender].index / baseline[age][gender].index);

                csv += age + "," + gender + ","
                    + baseline.total_unique_authors     + ","   + baseline[age][gender].unique_authors + ","    + baseline[age][gender].index + ","
                    + comparator.total_unique_authors   + ","   + comparator[age][gender].unique_authors + ","  + comparator[age][gender].index + ","
                    + baseline[age][gender].exp_baseline + "\n";
