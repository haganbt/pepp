 ...
 * @param strToCount - string to find and count
 * @returns {number}
 */
function countOccurrences(str, strToCount){
  let count = 0;
  let pos = str.indexOf(strToCount);

  while (pos !== -1) {
    count++;
    pos = str.indexOf(strToCount, pos + 1);
  }
      ...
      const traverse = function traverse(o) {
        for (const i in o) {
          ...
          if (isValidKey(i) === false && isInt(i) === false) {
            ...
            stats.data[v].interactions = stats.data[v].interactions || o[i].interactions || o[i][0].interactions;
            stats.data[v].unique_authors = stats.data[v].unique_authors || o[i].unique_authors || o[i][0].unique_authors;
      ...
      eachKey.map(key => {
        ...
        if (stats.data[lastKey]) {
          ...
          if(key.includes("__")){
            ...
            separatorCount = countOccurrences(key, "__");
            bumpKeyCount = true;
          }
