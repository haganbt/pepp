...
module.exports = {
    ...
    isValidKey: function (str) {
        let arr = ["plugin", "baseline", "filter", "interval", "span", "threshold", "target", "id", "name", "index", "analysis_type", "subscription_id", "api_resource", "only", "skip"];
     ...
     * @returns {*}
     */
    resUnixTsToHuman: function (analysis_type, responsePayload){
        try {

            if (analysis_type === "timeSeries") {
                for (let result in responsePayload) {

                    let ts = responsePayload[result].key;
