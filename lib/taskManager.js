...
const configCustomTags = config.has('customTags') ? config.get('customTags') : undefined;
...
const normalizeTask = function normalizeTask(tasks, analysis_type, mergedName, globalFilter) {
    ...
    tasks.map(function(task){
        ...
        if(_.has(task,'custom_tag')){
            requestParams.custom_tag = configCustomTags[task.custom_tag];
        } 
        ...
        if(_.has(task,'then')){
            ...
            if(_.has(task.then,'custom_tag')){
                requestParams.then.custom_tag = configCustomTags[task.then.custom_tag];
            }
