...
const buildRequest = function buildRequest(tasks, type, mergedName, globalFilter) {
    ...
    tasks.map(function(task){
        ...
        //load default request for either the task or analyze resource
        let reqObj = (config.get('app.api_resource') === "task") ? taskHelper.getDefaultTaskRequest() : taskHelper.getDefaultAnalyzeRequest();
        ...
        reqObj.json.parameters.parameters.parameters = _.clone(task);
        reqObj.json.parameters.parameters.analysis_type = task.type || type;
        ...
        if(_.has(task,'filter')){
            ...
            if(globalFilter){
                ...
                //todo - task api support
                reqObj.json.parameters.filter = "(" + _.clone(task.filter + ")");
        ...
        //todo /task specific
        delete reqObj.json.parameters.parameters.parameters.filter;
