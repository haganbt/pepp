...
const queueRequest = async function queueRequest(requestObj, normalizedTask) {
  ...
  const response = await request.make(requestObj);
  ...
  return await request.recursivePoll(normalizedTask, requestObj, response);
...
const responseHandler = function(normalizedTask, response) {
  ...
  if (_.has(normalizedTask, "then")) {
    ...
    nextTasks.map(each => {
      allRequests.push(queueRequest(requestFactory(each), each));
    });

    const d = Promise.all(allRequests);
