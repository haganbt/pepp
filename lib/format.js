...
function getBaselineKey(task) {
    ...
    const countTraverse = function countTraverse(task) {
        if (task.baseline) {
            hasBaseline = true;
...
function jsonToCsv(obj, task) {
    ...
    // baseline
        ...
        const baseLineKey = getBaselineKey(task); // the key to baseline against
    ...
    /**
        ...
        // k4 node
          ...
          // This could clearlly be improved.
                    const splits = y.split(/,[^, ]/g);
                    const x = splits[baseLineKey];
      ...
      // go through each of the class count totals and update each data item
            for (const idx in stats.class_count) {
                for (const d in stats.data) {
      ...
      // calculate baseline
            for (const eachObj in stats.data) {
