...
function gen(obj, task) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        //console.log(JSON.stringify(obj[0],undefined,4));
        ...
        //console.log(JSON.stringify(obj, undefined, 4));
        //console.log("Stats before:" + JSON.stringify(stats,undefined,4));
        traverse(obj);
        //console.log("Stats after traverse:" + JSON.stringify(stats,undefined,4));
        //calculate and inject probability in to stats obj
        stats = _calcAuthProbability(stats);
        //console.log("Stats after prob:" + JSON.stringify(stats,undefined,4));
        let baseline;
        ...
        //console.log("======================");
        //console.log("Stats*******" + JSON.stringify(stats, undefined, 4));

        let out = "key1,key2,key3,total_unique_authors,unique_author,probability,index,expected_baseline\n";
 ...
 * @returns {Promise.<T>}
 */
function nestedgen(obj, task) {
    //console.log("***********Before transform*****")
    //console.log(JSON.stringify(obj,undefined,4));
    obj = unnestresults(obj);
...
function unnestresults(nested_object) {
    var baseline;
    var new_object = [{}];

    Object.keys(nested_object[0]).map(function(k){
        if(k.includes('baseline')){
            baseline = nested_object[0][k];
            new_object[0][k] = baseline;
        }
        else {
            for (var z = 0; z < nested_object[0][k].length; z++){
                //var new_name = k + "_" + nested_object[0][k][z]["key"];
                var new_name = nested_object[0][k][z]["key"];
                var new_data = nested_object[0][k][z]["child"]["results"];
                new_object[0][new_name] = new_data;//
            }
        }
    });
    //console.log("*****unnesting function*****")
    //console.log(JSON.stringify(new_object,undefined,4));
    return new_object;
}

exports.gen = gen;
exports.nestedgen = nestedgen;
