...
const theCustomTags = config.has('customTags') ? config.get('customTags') : undefined;
/**
    ...
    tasks.map(function(task){
        ...
        */
        if(_.has(task,'custom_tag')){
            ...
            //requestParams.custom_tag = config.get('customTags.' + task.custom_tag);
            requestParams.custom_tag = theCustomTags[task.custom_tag];
            console.log("This is the custom tag: " + JSON.stringify(config.get('customTags.' + task.custom_tag),undefined,4));
        } 
        /****!!!
        ...
        */      
        ...
        if(_.has(task,'then')){
            ...
            if(_.has(task.then,'custom_tag')){
                console.log("The custom tag: " + task.then.custom_tag);

      requestParams.then.custom_tag = theCustomTags[task.then.custom_tag];
            }
