...
function getBaselineKey(task){
    ...
    let countTraverse = function countTraverse(task) {
        ...
        if(task.baseline){
            console.log("debug 1 = Found baseline in task at level:", count);
            return count;
        }
        ...
        for (var i in task) {
            if (task[i] !== null && typeof(task[i])=="object") {
                count ++;
                if(task[i].baseline){
                    console.log("debug 2 = Found baseline in task at level:", count);
                    return count;
                }
...
function jsonToCsv(obj, task) {
    ...
    return new Promise(resolve => {
        ...
        let baseLineKey = "k" + getBaselineKey(task);// the key to baseline against
        console.log("baseline key", baseLineKey);
        ...
        let traverse = function traverse(o) {
            for (var i in o) {
                ...
                if(o[i].key && !o[i].child){
                    ...
                    // get the actual value of the string key e.g. "k2"
                    let x = eval(baseLineKey);
                    console.log("Baseline grouping by:", x);
