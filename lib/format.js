...
function jsonToCsv(obj, task) {
  ...
  return new Promise(resolve => {
    ...
    const traverse = function traverse(o) {
      for (const i in o) {
        ...
        if (o[i].key && !o[i].child) {
          ...
          stats.data[y].interactions = stats.data[y].interactions ||
            o[i].interactions;
    ...
    stats.keys.map(key => {
      ...
      const keyCount = eachKey.length;
      let last = "";

      console.log("eachkey:", eachKey.length, eachKey);
      ...
      eachKey.map(key => {
        ...
        if (stats.data[lastKey]) {
          // add the interactions to the row
          csv += key + stats.data[lastKey].interactions + ",";

          // only build the header for the number of keys
          if (counter <= keyCount) {
            if (keyCount === counter) {
              header += `key_${counter},interactions,`;
            } else {
              header += `key_${counter},key_${counter}_interactions,`;
            }
          }
        } else {
