...
function gen(obj, task) {
  return new Promise(function(resolve, reject) {
    ...
    if (!task.json.parameters || !task.json.parameters.child) {
      reject(
        "Only native nested (child) tasks supported for Baseline calculations."
      );
    }

    if (
      task.json.parameters.analysis_type !== "freqDist" ||
      task.json.parameters.child.analysis_type !== "freqDist"
    ) {
    ...
    let traverse = function traverse(o) {
      for (let i in o) {
        if (isValidKey(i) === false && isInt(i) === false) {
          ...
          stats[prefix].total_unique_authors = stats[
            prefix
          ].total_unique_authors ||
            0;
        }
        ...
        if (o[i].key && !o[i].child) {
          ...
          stats[prefix][parent][h].unique_authors = stats[prefix][parent][
            h
          ].unique_authors ||
            o[i].unique_authors;
      ...
      Object.keys(stats[id]).map(function(k1) {
        ...
        if (baseline[k1]) {
          ...
          Object.keys(stats[id][k1]).map(function(k2) {
            ...
            if (baseline[k1][k2]) {
              baseline[k1][k2].index = stats[id][k1][k2].probability /
                baseline[k1][k2].probability;

              baseline[k1][k2].expected_baseline = stats[
                id
              ].total_unique_authors * baseline[k1][k2].probability;
              ...
              out += id +
                ...
                k2 +
                "," +
                stats[id].total_unique_authors +
                "," +
                stats[id][k1][k2].unique_authors +
                "," +
                stats[id][k1][k2].probability +
                "," +
                baseline[k1][k2].index +
                "," +
