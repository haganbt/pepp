 ...
 * @param "mergedName" - task name if a merged task or undefined
 */
const buildRequest = function buildRequest(tasks, type, mergedName) {
...
const loadConfigTasks = function loadConfigTasks(configTest) {
    ...
    for(let type in configObj){
        ...
        _.each(configObj[type], function (item) {
            ...
            allTasks.push(buildRequest(taskArr,type, mergedName));
        });
