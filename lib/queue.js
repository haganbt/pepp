...
const responseHandler = function(task, response){
    ...
    if (_.has(task, 'cache')) {
        ...
        log.error("Adding data -- ", task.cache.cacheId, task.cache.mergeKey, response.analysis.results);
        ...
        if (cacheData.remainingTasks === 0) {
            ...
            log.error("cache id ==== ",task.cache.cacheId);
...
const buildNextTasks = function(task, response){
    ...
    response.analysis.results.map(result => {
        ...
        nextTask.id = task.cache.mergeKey + "-" + result.key;
